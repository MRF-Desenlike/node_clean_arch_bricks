...
describe('{{module_name.pascalCase()}}Controller e2e tests', () => {
  ...
  describe('GET /{{module_folder_name}}/:id', () => {
    it('should get a item', async () => {
      const res = await request(app.getHttpServer())
        .get(`/{{module_folder_name}}/${entity._id}`)
        .set('Authorization', `Bearer `)
        .expect(200)
      const presenter = {{module_name.pascalCase()}}Controller.{{module_name.camelCase()}}ToResponse(entity.toJSON())
